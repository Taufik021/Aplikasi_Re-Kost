...
function createProductCard(p) {
    ...
    card.innerHTML = `
        <div class="product-img" style="background:#f8f9fa; display:flex; align-items:center; justify-content:center;">
            <img src="${p.image}" style="width:100%;height:100%;object-fit:contain; border-radius:15px 15px 0 0;">
        </div>
...
function renderChatList() {
    ...
    .onSnapshot(snapshot => {
        ...
        let docs = [];
        snapshot.forEach(doc => docs.push(doc));
        // Sort manual descending (terbaru di atas)
        docs.sort((a,b) => {
        ...
        docs.forEach(doc => {
            ...
            var date = new Date(chatData.timestamp || Date.now());
            ...
            div.innerHTML = `
                ...
                <div class="chat-info" style="flex:1; overflow:hidden;">
                    ...
                    <div class="chat-preview" style="color:#666; font-size:13px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${chatData.lastMessage || '📷 Mengirim lampiran'}</div>
                </div>
